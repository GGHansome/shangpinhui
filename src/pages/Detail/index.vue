          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd :class="{active:spuSaleAttrValue.isChecked == 1}" v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id"  @click="changeActive(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)">{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
              ...
              <div class="add">
                <!-- 以前咱们的路由跳转 从A路由跳转到B路由 这里加入购物车，进行路由跳转之前，发请求
                      把你购买的产品的信息通过请求的方式通知服务器，服务器进行相应的存储-->
  ...
  export default {
    ...
    methods: {
      ...
      async addShopCar(){
          ...
          let result = this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.number})
          ...
          try {
            if(await result == 200){
              ...
              //通过路由跳转携带params参数，并配合路由设置将params参数以props形式传递给跳转路由的组件
              /* let loction = {
                name:'addCartSuccess',
                query:{
                  skuNum:this.number
